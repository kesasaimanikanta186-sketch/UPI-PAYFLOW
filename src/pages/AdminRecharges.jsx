// AdminRecharges.jsx - Admin view of all recharges
import { useState, useEffect } from 'react'
import { useNavigate } from 'react-router-dom'
import api from '../services/api'
import LoadingSpinner from '../components/LoadingSpinner'
import { IoArrowBack, IoPhonePortraitOutline, IoCheckmarkCircle, IoCloseCircle, IoTimeOutline } from 'react-icons/io5'
...
function AdminRecharges() {
  ...
  function loadRecharges() {
    setLoading(true)
    api.get('/api/recharge/history/all?page=' + page + '&per_page=20')
      .then(function (res) {
  ...
  return (
    <div className="page-container space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Recharge History</h2>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {recharges.map(function (rec) {
            var isSuccess = rec.status === 'success'
            var isPending = rec.status === 'pending'
            return (
              <div key={rec.id} className="card animate-fadeIn">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {rec.mobile_number} • {rec.operator}
                      </p>
                      <p className="text-[10px] text-gray-400 mt-1">
                        {new Date(rec.date_time).toLocaleString('en-IN')} • {rec.transaction_id}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-gray-900 dark:text-white">{'\u20B9'}{rec.amount.toLocaleString('en-IN')}</p>
